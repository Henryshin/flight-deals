...
**변동 항목** 보드의 1열 `★` 로 마음에 드는 일정을 담아 두면, 페이지 최하단
**★ 즐겨찾기** 보드에 모아서 볼 수 있습니다. 담긴 목록은 브라우저의
`localStorage`(`fd_deal_favs`)에만 저장됩니다 — 서버/저장소에 올라가지 않습니다.

- 담는 단위는 **모니터 x 출발일 x 귀국일** 입니다 (여행지별 보드의 `★ 내 목록`은
  노선 단위라 별도 저장소 `fd_favs` 를 씁니다). 편명·출발시각은 수집마다 바뀔 수
  있어 키에 넣지 않습니다.
- 담을 때 그 항목의 값을 통째로 저장하고, 이후 수집에서 같은 항목이 있으면 최신값으로
  갱신합니다. 기준(30일 평균 대비 15% 하락)에서 빠져 `deals.json` 에 더 이상 없으면
  지우지 않고 **기준 미달** 로 흐리게 남깁니다 — 담아 둔 항목이 소리 없이 사라지지
  않게 하기 위함입니다.
- 위쪽 필터(국가·연휴·최대 가격·최대 연차)는 이 보드에 걸리지 않습니다. 담아 둔 것을
  필터 때문에 못 찾는 일이 없도록, 정렬(담은 순/할인율/현재값/출발일)만 제공합니다.
